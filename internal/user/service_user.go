...
func buildUserUpdateMsg(
	userID int64,
	accountType string,
	nickname, avatar, gender, signature string,
) TopicUserUpdateMsg {
	return TopicUserUpdateMsg{
...
func (s *Service) publishUserUpdate(ctx context.Context, userID int64, msg TopicUserUpdateMsg, scope string) {
	if s.producer == nil {
		return
	}

	if err := s.producer.SendTopicUserUpdate(ctx, msg); err != nil {
		s.logger.Warn("send topic user update mq failed", zap.Error(err), zap.String("scope", scope), zap.Int64("userID", userID))
	}
	if err := s.producer.SendCommentUserUpdate(ctx, CommentUserUpdateMsg(msg)); err != nil {
		s.logger.Warn("send comment user update mq failed", zap.Error(err), zap.String("scope", scope), zap.Int64("userID", userID))
	}
}
